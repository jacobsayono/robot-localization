...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	// can also use the .push_back() method, then no need to initialize newGrid with size
	// but must create 2 new vectors for new_rows and new_cols
  ...
  float blurring) 
{
  ...
  // your code here
	...
	for (int r = 0; r < height; ++r) {
		for (int c = 0; c < width; ++c) {
			int new_r = ((r + dy) % height + height) % height;
			int new_c = ((c + dx) % width + width) % width;
			newGrid[(int)new_r][(int)new_c] = beliefs[r][c];
		}
	}
	...
	float p_miss) 
{
	...
	// your code here
	for (int r = 0; r < beliefs.size(); ++r) {
		for (int c = 0; c < beliefs[0].size(); ++c) {
			bool hit = (color == grid[r][c]);
			newGrid[r][c] = beliefs[r][c] * (hit * p_hit + (1 - hit) * p_miss);
		}
	}
